...
template<typename modeltype>
template<class algo, typename>
inline void rts::rtsModelOptim<modeltype>::ml_beta_theta(){  
  ...
  if constexpr (std::is_same_v<algo,LBFGS>){
    ...
    if constexpr (std::is_same_v<modeltype,BitsHSGP>) {
      op.template fn<&rts::rtsModelOptim<BitsHSGP>::log_likelihood_beta_theta, rts::rtsModelOptim<BitsHSGP> >(this);
    } else {
...
template<typename modeltype>
inline double rts::rtsModelOptim<modeltype>::log_likelihood_beta_theta(const dblvec& pars)
{
...
template<>
inline double rts::rtsModelOptim<BitsHSGP>::log_likelihood_beta_theta(const dblvec& pars){
